...
def s_cross_validate(K, x, y, Algorithm, parameters):
    ...
    for k in range(K):
        ...
        for i, params in enumerate(parameters):
            l = Algorithm(params)
            l.rbfn(train_data,train_label)
            pred = l.rbfn_test(val_data)
            acc = g_accuracy(val_label,pred)
            all_acc[i,k] = acc
...
if __name__ == '__main__':
    ...
    #     new_y_train = np.reshape(new_y_train,(60000,1))
    #     new_y_test = np.reshape(new_y_test,(10000,1))
